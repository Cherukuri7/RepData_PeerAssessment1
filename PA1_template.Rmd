...
````
The mean total number of steps taken per day is **`r as.integer(stepsmean)`**. The median is **`r as.integer(stepsmedian)`**.
...
The line graph indicates that the maximum number of steps, **`r as.integer(peak$steps)`**, are taken at 5-minute interval of **`r peak$interval`**.
...
```
The total number of rows with missing values (NA s) is **`r countNAs`**.
...
The mean total number of steps taken per day is **`r as.integer(stepsmean2)`**. The median is **`r as.integer(stepsmedian2)`**. The estimates of the total daily number of steps remained very similar because we replaced all missing values with the average values for each 5-minute interval.
